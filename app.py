...
with st.sidebar:
    ...
    st.markdown(
        ...
        "margin-top:6px;'>or upload a WAV / MP3 file</div>",
        unsafe_allow_html=True
    )
    ...
    # ── Process JS audio if postMessage came through ──────────
...
vital_card(v1, "Heart Rate",       hr,         "BPM",
           vitals["hr_status"],
           "#10b981" if vitals["hr_status"]   == "Normal"  else "#f59e0b")
vital_card(v2, "SpO2",             f"{spo2}%", "Blood Oxygen",
           vitals["spo2_status"],
...
vital_card(v4, "HRV RMSSD",        rmssd,      "Milliseconds",
           vitals["hrv_status"],
           "#10b981" if rmssd >= 40 else "#f59e0b" if rmssd >= 25 else "#ef4444")
vital_card(v5, "LF/HF Ratio",      lf_hf,      "Autonomic Balance",
           vitals["lfhf_status"],
...
with a1:
    ...
    <div class='analysis-card'>
        ...
        <div class='analysis-card-sub'>
            Confidence: {stress["confidence"]}% &nbsp;|&nbsp; WESAD Random Forest
        </div>
...
with t1:
    st.markdown(
        ...
        "HRV RMSSD — 3 Day Trend</div>",
        unsafe_allow_html=True
    )
    st.plotly_chart(make_trend(hrv_history, "rmssd", "#3b82f6"), use_container_width=True)

    st.markdown(
        "<div style='font-size:0.72rem;color:#475569;margin-bottom:6px;margin-top:8px;'>"
...
with t3:
    ...
    <div class='cycle-info-card'>
        ...
        <div class='cycle-meta'>
            Confidence: {cycle["confidence"]}% &nbsp;|&nbsp; cilab-ufersa + Oura Research
        </div>
...
if normal_tips:
    ...
    for i, tip in enumerate(normal_tips):
        ...
        <div class='suggestion-card'>{tip["text"]}</div>""",
        unsafe_allow_html=True)
...
with st.expander("Raw Sensor Data — Developer View"):
    raw_df = pd.DataFrame([
        {"Metric": "Heart Rate",    "Value": hr,               "Unit": "BPM",
         "Range": "60–100",    "Status": "Normal"   if 60 <= hr <= 100      else "Check"},
        {"Metric": "SpO2",          "Value": spo2,             "Unit": "%",
         "Range": "95–100",    "Status": "Healthy"  if spo2 >= 95           else "Low"},
        {"Metric": "HRV RMSSD",     "Value": rmssd,            "Unit": "ms",
         "Range": "40–80",     "Status": "Good"     if rmssd >= 40          else "Reduced"},
        {"Metric": "HRV SDNN",      "Value": sdnn,             "Unit": "ms",
         "Range": "30–70",     "Status": "Good"     if sdnn >= 30           else "Reduced"},
        {"Metric": "LF/HF Ratio",   "Value": lf_hf,            "Unit": "ratio",
         "Range": "1.0–2.5",   "Status": "Balanced" if lf_hf <= 2.5        else "Elevated"},
        {"Metric": "Skin Temp",     "Value": temp_c,           "Unit": "C",
         "Range": "36.0–37.5", "Status": "Normal"   if 36 <= temp_c <= 37.5 else "Elevated"},
        {"Metric": "Stress Score",  "Value": stress["score"],  "Unit": "0–1",
         "Range": "< 0.35",    "Status": stress["label"]},
        {"Metric": "Fatigue Score", "Value": fatigue["score"], "Unit": "0–1",
         "Range": "< 0.35",    "Status": fatigue["label"]},
        {"Metric": "Distress Score","Value": distress_score,   "Unit": "0–1",
         "Range": "< 0.35",    "Status": emotion_sel},
        {"Metric": "Cycle Day",     "Value": cycle_day,        "Unit": "day",
         "Range": f"1–{cycle_len}", "Status": cycle["phase"]},
    ])
