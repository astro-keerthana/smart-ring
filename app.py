...
from core.audio_emotion import get_result, analyse
...
def make_gauge(value, label, color, max_val=100):
    fig = go.Figure(go.Indicator(
        ...
        number = {"font": {"color": color, "size": 30, "family": "Inter"}, "suffix": ""},
        gauge  = {
            "axis"      : {"range": [0, max_val], "tickcolor": "#1e2d45",
                           "tickfont": {"color": "#334155", "size": 9}, "nticks": 5},
...
def make_trend(history, key, color):
    ...
    fig.add_trace(go.Scatter(
        ...
        marker    = dict(size=5, color=color, line=dict(color="#0f1829", width=1.5)),
        fill      = "tozeroy",
        fillcolor = fill_rgba,
    ))
    fig.update_layout(
...
def make_bar_probs(relaxed, stressed, amused):
    fig = go.Figure(go.Bar(
        ...
        marker_line_width = 0,
        text              = [f"{relaxed*100:.0f}%", f"{stressed*100:.0f}%", f"{amused*100:.0f}%"],
    ...
    fig.update_layout(
        ...
        yaxis=dict(gridcolor="#1e2d45", tickfont=dict(size=9), range=[0,115]),
        **CHART_DEFAULTS
    )
    return fig
...
def make_cycle_donut(day, total, color):
    ...
    fig = go.Figure(go.Pie(
        values=[day, remaining], labels=["Elapsed","Remaining"],
        hole=0.72, marker_colors=[color,"#1e2d45"],
        textinfo="none", hoverinfo="label+value",
        direction="clockwise", sort=False,
    ))
    fig.add_annotation(
        text=f"<b>Day {day}</b><br><span style='font-size:10px'>of {total}</span>",
        x=0.5, y=0.5,
        font=dict(size=16, color="#f1f5f9", family="Inter"),
        showarrow=False, align="center"
    )
    fig.update_layout(height=200, showlegend=False, **CHART_DEFAULTS)
    return fig
...
with st.sidebar:
    ...
    <div style='padding:8px 0 16px 0;'>
        <div style='font-size:1.0rem;font-weight:700;color:#f1f5f9;letter-spacing:-0.02em;'>
            Smart Ring
        </div>
    ...
    label_color = (
        "#10b981" if ar["label"] in ("Calm", "Happy", "Not Recorded")
        else "#f59e0b" if ar["label"] in ("Neutral", "Sad", "No Voice")
        else "#ef4444"
    )
    ...
    <div style='background:#0f1829;border:1px solid #1e2d45;
        ...
        <div style='font-size:1.2rem;font-weight:700;
                    color:{label_color};margin-top:4px;'>
            {ar["label"]}
        </div>
    ...
    distress_score = emotion_map.get(ar["label"], 0.10)
...
suggestions = get_suggestions(stress, fatigue, spo2, hr, temp_c, cycle["phase"])
...
# ── Status Banner ─────────────────────────────────────────────
if overall > 0.65:
    bc,bd,bt,bs = ("banner-critical","banner-dot-critical","Attention Required",
                   "Elevated stress and fatigue detected. Immediate rest and recovery is recommended.")
elif overall > 0.35:
    bc,bd,bt,bs = ("banner-warning","banner-dot-warning","Moderate Alert",
                   "Physiological indicators suggest mild stress accumulation. Monitor and take short recovery breaks.")
else:
    bc,bd,bt,bs = ("banner-normal","banner-dot-normal","All Systems Normal",
                   "All physiological indicators are within healthy ranges. Current state is well-regulated.")
...
def vital_card(col, label, value, unit, status_text, accent):
    col.markdown(f"""
    <div class='metric-card metric-card-{"green" if accent=="#10b981" else "yellow" if accent=="#f59e0b" else "red"}'>
        <div class='metric-card-label'>{label}</div>
        <div class='metric-card-value' style='color:{accent};'>{value}</div>
        <div class='metric-card-unit'>{unit}</div>
        <div class='metric-card-status {"status-normal" if accent=="#10b981" else "status-warning" if accent=="#f59e0b" else "status-critical"}'>{status_text}</div>
    </div>
    """, unsafe_allow_html=True)

vital_card(v1, "Heart Rate",       hr,        "BPM",            vitals["hr_status"],
           "#10b981" if vitals["hr_status"]   == "Normal"  else "#f59e0b")
vital_card(v2, "SpO2",             f"{spo2}%","Blood Oxygen",   vitals["spo2_status"],
           "#10b981" if vitals["spo2_status"] == "Healthy" else "#ef4444")
vital_card(v3, "Skin Temperature", f"{temp_c}","Celsius",
           "Normal" if 36.0 <= temp_c <= 37.5 else "Elevated",
           "#10b981" if 36.0 <= temp_c <= 37.5 else "#f59e0b")
vital_card(v4, "HRV RMSSD",        rmssd,     "Milliseconds",   vitals["hrv_status"],
           "#10b981" if rmssd >= 40 else "#f59e0b" if rmssd >= 25 else "#ef4444")
vital_card(v5, "LF/HF Ratio",      lf_hf,     "Autonomic Balance", vitals["lfhf_status"],
...
with a1:
    ...
    st.plotly_chart(make_gauge(int(stress["score"]*100), "Stress Level", sc), use_container_width=True)
    st.markdown(f"""
    <div class='analysis-card'>
        <div class='analysis-card-title'>Stress Detection</div>
        <div class='analysis-card-result' style='color:{sc};'>{stress["label"]}</div>
        <div class='analysis-card-sub'>Confidence: {stress["confidence"]}% &nbsp;|&nbsp; WESAD Random Forest</div>
    </div>""", unsafe_allow_html=True)

with a2:
    fc = score_color(fatigue["score"])
    st.plotly_chart(make_gauge(int(fatigue["score"]*100), "Fatigue Level", fc), use_container_width=True)
    st.markdown(f"""
    <div class='analysis-card'>
        <div class='analysis-card-title'>Fatigue Detection</div>
        <div class='analysis-card-result' style='color:{fc};'>{fatigue["label"]}</div>
        <div class='analysis-card-sub'>HRV decline: {fatigue["decline_pct"]}% over 3 days</div>
    </div>""", unsafe_allow_html=True)

with a3:
    dc = score_color(distress_score)
    st.plotly_chart(make_gauge(int(distress_score*100), "Voice Distress", dc), use_container_width=True)
    st.markdown(f"""
    <div class='analysis-card'>
        <div class='analysis-card-title'>Audio Emotion</div>
        <div class='analysis-card-result' style='color:{dc};'>{emotion_sel}</div>
        <div class='analysis-card-sub'>librosa &nbsp;|&nbsp; RMS + ZCR + MFCC features</div>
    </div>""", unsafe_allow_html=True)

with a4:
    st.plotly_chart(make_bar_probs(stress["prob_relaxed"], stress["prob_stressed"], stress["prob_amused"]),
...
# ── Trends + Cycle ────────────────────────────────────────────
st.markdown("<div class='section-label'>Trends and Cycle</div>", unsafe_allow_html=True)
t1, t2, t3 = st.columns([2, 2, 2])

with t1:
    st.markdown("<div style='font-size:0.72rem;color:#475569;margin-bottom:6px;'>HRV RMSSD — 3 Day Trend</div>",
                unsafe_allow_html=True)
    st.plotly_chart(make_trend(hrv_history, "rmssd", "#3b82f6"), use_container_width=True)
    st.markdown("<div style='font-size:0.72rem;color:#475569;margin-bottom:6px;margin-top:8px;'>Heart Rate — 3 Day Trend</div>",
                unsafe_allow_html=True)
    st.plotly_chart(make_trend(hrv_history, "hr", "#8b5cf6"), use_container_width=True)

with t2:
    hours  = ["06:00","08:00","10:00","12:00","14:00","16:00","18:00","Now"]
    s_vals = [0.15,0.22,0.41,0.55,0.48,0.62,0.70, round(stress["score"],2)]
    fig_s  = go.Figure()
    fig_s.add_trace(go.Scatter(
        x=hours, y=s_vals, mode="lines+markers",
        line=dict(color="#ef4444", width=1.8, shape="spline"),
        marker=dict(size=5, color="#ef4444", line=dict(color="#0f1829", width=1.5)),
        fill="tozeroy", fillcolor="rgba(239,68,68,0.08)",
    ))
    fig_s.update_layout(height=140,
        xaxis=dict(showgrid=False, tickfont=dict(size=9)),
        yaxis=dict(gridcolor="#1e2d45", tickfont=dict(size=9), range=[0,1.1]),
        **CHART_DEFAULTS)
    st.markdown("<div style='font-size:0.72rem;color:#475569;margin-bottom:6px;'>Stress Score — Today</div>",
                unsafe_allow_html=True)
    ...
    fig_o.add_trace(go.Scatter(
        x=hours, y=spo2_vals, mode="lines+markers",
        line=dict(color="#10b981", width=1.8, shape="spline"),
        marker=dict(size=5, color="#10b981", line=dict(color="#0f1829", width=1.5)),
        fill="tozeroy", fillcolor="rgba(16,185,129,0.08)",
    ))
    fig_o.update_layout(height=140,
        xaxis=dict(showgrid=False, tickfont=dict(size=9)),
        yaxis=dict(gridcolor="#1e2d45", tickfont=dict(size=9), range=[85,102]),
        **CHART_DEFAULTS)
    st.markdown("<div style='font-size:0.72rem;color:#475569;margin-bottom:6px;margin-top:8px;'>SpO2 — Today</div>",
                unsafe_allow_html=True)
    st.plotly_chart(fig_o, use_container_width=True)

with t3:
    st.markdown("<div style='font-size:0.72rem;color:#475569;margin-bottom:6px;'>Menstrual Cycle</div>",
                unsafe_allow_html=True)
    st.plotly_chart(make_cycle_donut(cycle_day, cycle_len, "#8b5cf6"), use_container_width=True)
    st.markdown(f"""
    <div class='cycle-info-card'>
        <div class='cycle-phase-label'>{cycle["phase"]} Phase</div>
        <div class='cycle-meta'>Confidence: {cycle["confidence"]}% &nbsp;|&nbsp; cilab-ufersa + Oura Research</div>
        <div class='cycle-meta' style='margin-top:4px;color:#94a3b8;'>{cycle["description"]}</div>
        <div class='cycle-next'>
            Next period: <strong>{cycle["next_period"]}</strong>
            &nbsp; ({cycle["days_left"]} days remaining)<br>
...
if high_tips:
    s1, s2 = st.columns(2)
    for i, tip in enumerate([s1, s2]):
        if i < len(high_tips):
            tip.markdown(f"""
            <div class='suggestion-card suggestion-card-priority'>
                <div style='font-size:0.68rem;font-weight:700;color:#ef4444;
...
if normal_tips:
    ...
    for i, tip in enumerate(normal_tips):
        ...
        <div class='suggestion-card'>{tip["text"]}</div>""", unsafe_allow_html=True)
...
with st.expander("Raw Sensor Data — Developer View"):
    raw_df = pd.DataFrame([
        {"Metric":"Heart Rate",    "Value":hr,              "Unit":"BPM",   "Range":"60–100",    "Status":"Normal" if 60<=hr<=100 else "Check"},
        {"Metric":"SpO2",          "Value":spo2,            "Unit":"%",     "Range":"95–100",    "Status":"Healthy" if spo2>=95 else "Low"},
        {"Metric":"HRV RMSSD",     "Value":rmssd,           "Unit":"ms",    "Range":"40–80",     "Status":"Good" if rmssd>=40 else "Reduced"},
        {"Metric":"HRV SDNN",      "Value":sdnn,            "Unit":"ms",    "Range":"30–70",     "Status":"Good" if sdnn>=30 else "Reduced"},
        {"Metric":"LF/HF Ratio",   "Value":lf_hf,           "Unit":"ratio", "Range":"1.0–2.5",   "Status":"Balanced" if lf_hf<=2.5 else "Elevated"},
        {"Metric":"Skin Temp",     "Value":temp_c,          "Unit":"C",     "Range":"36.0–37.5", "Status":"Normal" if 36<=temp_c<=37.5 else "Elevated"},
        {"Metric":"Stress Score",  "Value":stress["score"], "Unit":"0–1",   "Range":"< 0.35",    "Status":stress["label"]},
        {"Metric":"Fatigue Score", "Value":fatigue["score"],"Unit":"0–1",   "Range":"< 0.35",    "Status":fatigue["label"]},
        {"Metric":"Distress Score","Value":distress_score,  "Unit":"0–1",   "Range":"< 0.35",    "Status":emotion_sel},
        {"Metric":"Cycle Day",     "Value":cycle_day,       "Unit":"day",   "Range":f"1–{cycle_len}", "Status":cycle["phase"]},
    ])
...
# ── Footer ────────────────────────────────────────────────────
st.markdown("""
<div style='padding:24px 0 8px 0;text-align:center;font-size:0.72rem;color:#334155;
